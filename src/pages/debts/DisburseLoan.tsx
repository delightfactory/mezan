      ...
      <form onSubmit={handleSubmit} className="space-y-5 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none transition-all text-left text-emerald-600 font-bold text-xl"
            dir="ltr"
            placeholder="0.00"
            required
          />
        ...
        <div>
          <label className="block text-sm font-bold text-gray-700 mb-2">سحب الفلوس من محفظة</label>
          <select
            value={walletId}
            onChange={(e) => setWalletId(e.target.value)}
            className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none transition-all bg-white"
            required
          >
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <button
                  ...
                  className={`px-3 py-2 text-xs font-bold rounded-lg border transition-all ${scheduleType === 'FLEXIBLE' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                >
                  مرن (حسب المقدرة)
                </button>
                <button
                  ...
                  className={`px-3 py-2 text-xs font-bold rounded-lg border transition-all ${scheduleType === 'MONTHLY_INSTALLMENT' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                >
                  قسط شهري
                </button>
                <button
                  ...
                  className={`px-3 py-2 text-xs font-bold rounded-lg border transition-all ${scheduleType === 'ONE_TIME' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                >
                  دفعة واحدة
                </button>
              </div>
            ...
            {(scheduleType === 'MONTHLY_INSTALLMENT' || scheduleType === 'ONE_TIME') && (
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none transition-all text-left"
                  dir="ltr"
                />
              </div>
            )}
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full bg-emerald-600 text-white font-bold py-4 rounded-xl shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition-all active:scale-95 disabled:opacity-70 mt-4"
        >
