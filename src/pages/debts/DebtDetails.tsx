...
import { ArrowRight, History, Calendar, Trash2, ShieldAlert } from 'lucide-react';
...
export const DebtDetails: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      setLoading(true);
      const [fetchedDebts, fetchedEvents] = await Promise.all([
  ...
  const handleReschedule = async (e: React.FormEvent) => {
    ...
    try {
      await debtService.rescheduleDebt({
        ...
        p_next_due_date: nextDueDate ? nextDueDate : undefined,
      ...
      <div className={`p-6 rounded-3xl text-white shadow-lg ${isSettled ? 'bg-gray-800 shadow-gray-200' : isOwedByUs ? 'bg-gradient-to-br from-rose-500 to-rose-700 shadow-rose-200' : 'bg-gradient-to-br from-emerald-500 to-emerald-700 shadow-emerald-200'}`}>
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-white/80 text-sm font-bold opacity-90">
              {debt.debt_kind === 'GAMEYA' ? 'جمعية' : debt.debt_kind === 'WORK_ADVANCE' ? 'سلفة عمل' : isOwedByUs ? 'دين علينا' : 'مستحق لنا'}
            </p>
          </div>
          <div className="bg-white/20 px-3 py-1 rounded-full text-xs font-bold backdrop-blur-sm">
            {debt.status === 'WRITTEN_OFF' ? 'مقطوع الأمل (مشطوب)' : debt.status === 'SETTLED' ? 'تمت التسوية' : 'نشط'}
          </div>
        </div>
        ...
        {!isSettled && (
          <div className="flex space-x-2 space-x-reverse mt-2">
            <Link to={`/debts/${debt.id}/payment`} className="flex-1 bg-white text-gray-900 text-center py-3 rounded-xl font-bold text-sm hover:bg-gray-50 transition-colors active:scale-95 shadow-sm">
              {isOwedByUs ? 'سداد قسط' : 'تحصيل دفعة'}
            </Link>
          </div>
        )}
      </div>
              ...
              <form id="rescheduleForm" onSubmit={handleReschedule} className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none transition-all bg-white font-bold"
                  >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-indigo-500 outline-none transition-all font-bold"
                        required
                      />
                    </div>
                    <div>
                ...
                {(scheduleType === 'MONTHLY_INSTALLMENT' || scheduleType === 'ONE_TIME') && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-indigo-500 outline-none transition-all text-left font-bold"
                      required={scheduleType === 'MONTHLY_INSTALLMENT'}
                    />
                  </div>
                )}
              </form>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
            ...
            <div className="p-6">
              <p className="text-sm font-bold text-gray-700 mb-4 leading-relaxed">
                ...
                لا يمكن التراجع عن هذا الإجراء وسيتم تصفير المتبقي وتسجيله كمبلغ مشطوب.
              </p>
              <form id="writeOffForm" onSubmit={handleWriteOff}>
                <label className="block text-sm font-bold text-gray-700 mb-2">سبب الشطب / ملاحظات (اختياري)</label>
                <textarea
                  value={writeOffNotes}
                  onChange={(e) => setWriteOffNotes(e.target.value)}
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-rose-500 focus:ring-2 focus:ring-rose-100 outline-none transition-all resize-none h-24"
                  placeholder="مثال: التنازل عن المتبقي، عدم القدرة على السداد..."
                />
              </form>
            </div>
            <div className="p-4 border-t border-gray-100 flex gap-3 bg-gray-50">
