  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const incomeCategories = fetchedCategories.filter((category) => category.direction === 'INCOME' && !category.is_archived);
  ...
  return (
    <div className="space-y-6">
      <div className="mb-6 flex items-center gap-3">
        ...
        <h2 className="text-xl font-bold text-gray-900">إضافة دخل</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        <div>
          ...
          <input
            ...
            className="w-full rounded-xl border border-gray-200 px-4 py-3 text-left text-xl font-bold text-green-600 outline-none transition-all focus:border-green-500 focus:ring-2 focus:ring-green-100"
            dir="ltr"
            placeholder="0.00"
            required
          />
        ...
        <div>
          ...
          <select value={categoryId} onChange={(event) => setCategoryId(event.target.value)} className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 outline-none transition-all focus:border-green-500 focus:ring-2 focus:ring-green-100" required>
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-xl border border-gray-200 px-4 py-3 outline-none transition-all focus:border-green-500 focus:ring-2 focus:ring-green-100"
            placeholder="مثال: راتب الشهر"
          />
        </div>

        <button type="submit" disabled={submitting} className="mt-4 w-full rounded-xl bg-green-600 py-4 font-bold text-white transition-colors hover:bg-green-700 disabled:opacity-70">
          {submitting ? 'جاري الحفظ...' : 'تأكيد إضافة الدخل'}
        </button>
      </form>
    </div>
  );
